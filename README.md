...
Download best_model.pt from the original repository.
...
Geometry Types
      ...
      Point_2D
      2D point (for E² and H²)
      X, Y : Real
    
    
      Point_3D
      3D point (for S²)
      X, Y, Z : Real with X² + Y² + Z² = 1.0
    
    
      Universal_Point
      Discriminated union for all geometries
      G : Geometry_Type
    
    
      Polygon
      Cyclic sequence of points
      N : Polygon_Index
...
Key Functions
      ...
      Classical_4Point_Subdivide
      Classical 4-point scheme
      All
      ✅
    
    
      Angle_Based_Insert
      Insert vertex using angle α
      All
      ✅
    
    
      Clamp_Alpha
      Enforce G¹ safety bound (Theorem 1)
      All
      ✅
...
What’s Replaced
      ...
      Original (Python)
      This Project (Ada SPARK)
    
  
  
    
      Neural network (NeuralOperator)
      Lookup tables (alpha_table.bin)
    
    
      Dynamic weight prediction
      Static angle values (from best_model.pt)
    
    
      Training loop
      Not applicable (use pre-trained weights)
...
Original Repository: ugail/neural_subdivision
...
The original code is released for research and academic use (see ugail/neural_subdivision).
